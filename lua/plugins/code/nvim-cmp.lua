return {
  ...
  opts = function()
    ...
    if not cmp_active then return end

    local lspkind_active, lspkind = pcall(require, "lspkind")
    if not lspkind_active then return end

    local cmp_autopairs_active, cmp_autopairs = pcall(require, "nvim-autopairs.completion.cmp")
    if not cmp_autopairs_active then return end

    local cmp_tailwind_active, cmp_tailwind = pcall(require, "tailwindcss-colorizer-cmp")
    if not cmp_tailwind_active then return end

    local cmp_ai_active, cmp_ai = pcall(require, "cmp_ai")
    if not cmp_ai_active then return end
